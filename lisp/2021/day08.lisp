(defun count-one-four-seven-eight (input)
  (loop for (a b) on input by #'cddr
        sum (loop for n in b when (member (length (string n)) '(2 3 4 7)) sum 1)))

(defun decipher-and-sum (input)
  (loop for (letters numbers) on input by #'cddr
        for l = (letters-to-segments letters)
        for n = (letters-to-segments-unsorted numbers)
        for s = (apply #'find-segments l)
        sum (loop for x in n with sum = 0
                  do (setf sum (+ (sequence-to-number x s) (* 10 sum)))
                  finally (return sum))))

(defun letters-to-segments (letters)
  (sort (letters-to-segments-unsorted letters) (lambda (a b) (< (length a) (length b)))))

(defun letters-to-segments-unsorted (letters)
  (loop for word in letters
        collect (sort (loop for letter across (string word) collect (string letter))
                      #'string-lessp)))

(defun find-segments (one seven four two three five six nine zero eight)
  (declare (ignore six nine zero))
  (let* ((real-three-five (find-three-five two three four five seven eight))
         (a (find-a one seven))
         (b-d (find-b-d one four))
         (e-g (find-e-g four seven eight))
         (b-c (apply #'find-b-c real-three-five))
         (f (find-f a b-c b-d e-g eight))
         (b (find-b b-c b-d))
         (c (find-c one f))
         (d (find-d b-d b))
         (g (apply #'find-g (append real-three-five (list (append a b c d f)))))
         (e (find-e e-g g)))
    (append a b c d e f g)))

(defun sequence-to-number (sequence segments)
  (destructuring-bind (a b c d e f g) segments
    (cond ((equal sequence (letter-segment (list c f))) 1)
          ((equal sequence (letter-segment (list a c f))) 7)
          ((equal sequence (letter-segment (list b c d f))) 4)
          ((equal sequence (letter-segment (list a c d e g))) 2)
          ((equal sequence (letter-segment (list a c d f g))) 3)
          ((equal sequence (letter-segment (list a b d f g))) 5)
          ((equal sequence (letter-segment (list a b d e f g))) 6)
          ((equal sequence (letter-segment (list a b c d f g))) 9)
          ((equal sequence (letter-segment (list a b c e f g))) 0)
          ((equal sequence (letter-segment (list a b c d e f g))) 8))))

(defun letter-segment (list)
  (sort list #'string-lessp))

(defun set-diff (a b)
  (set-difference a b :test #'equal))

(defun set-union (&rest l)
  (loop for i in l
        for f = i then (union f i :test #'equal)
        finally (return f)))

(defun set-inter (a b)
  (intersection a b :test #'equal))

(defun find-a (one seven)
  (set-diff seven one))

(defun find-b-d (one four)
  (set-diff four one))

(defun find-two (two three four five seven eight)
  (loop for i in (list two three five)
        when (is-two i four seven eight) return i))

(defun find-three-five (two three four five seven eight)
  (let ((real-two (find-two two three four five seven eight)))
    (set-diff (list two three five) (list real-two))))

(defun find-e-g (four seven eight)
  (set-diff eight (set-union four seven)))

(defun is-two (two four seven eight)
  (let ((e-g (letter-segment (find-e-g four seven eight))))
    (equal e-g (letter-segment (set-inter two e-g)))))

(defun find-b-c (three five)
  (set-diff (set-union three five) (set-inter three five)))

(defun find-f (a b-c b-d e-g eight)
  (set-diff eight (set-union e-g b-d a b-c)))

(defun find-b (b-c b-d)
  (set-inter b-c b-d))
...
(defun find-g (three five a-b-c-d-f)
  (set-diff (set-union three five) a-b-c-d-f))

(defun find-e (e-g g)
  (set-diff e-g g))

(defun day08-1 () (assert (= 278 (count-one-four-seven-eight (input)))))

(defun day08-2 () (assert (= 986179 (decipher-and-sum (input)))))
