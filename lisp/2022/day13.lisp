(defun is-correct (a b)
  (cond
    ((and (numberp a) (numberp b))
     (if (< a b) 'yes (if (> a b) 'no 'not-known)))
    ((and (listp a) (listp b))
     (cond
      ((and (null b) (not (null a))) 'no)
      ((and (null a) (not (null b))) 'yes)
      (t
        (loop for a-elem in a
              for b-elem in b
              for correct = (is-correct a-elem b-elem)
              when (or (equal correct 'no) (equal correct 'yes))
                do (return correct)
              finally
                 (return
                   (if (< (length a) (length b)) 'yes
                       (if (> (length a) (length b)) 'no 'not-known)))))))
   (t
    (let ((a-list (if (listp a) a (list a)))
          (b-list (if (listp b) b (list b))))
      (is-correct a-list b-list)))))

(defun get-right-order-sum (input)
  (apply #'+ (loop for i = 1 then (1+ i)
                   for (a b) on input by #'cddr
                   when (equal 'yes (is-correct a b))
                     collect i)))

(defun sort-it (input)
  (sort (append '(((2)) ((6))) (copy-tree input)) #'(lambda (x y) (equal 'yes (is-correct x y)))))

(defun get-decoder-key (input)
  (let ((ordered (sort-it input)))
    (* (1+ (position '((2)) ordered :test #'equal))
       (1+ (position '((6)) ordered :test #'equal)))))

(defun day13-part1 ()
  (assert (equal 5843 (get-right-order-sum (input)))))

(defun day13-part2 ()
  (assert (equal 26289 (get-decoder-key (input)))))
