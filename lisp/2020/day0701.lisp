(defun day-07-2020-1 ()
  (let ((key '(("shiny" "gold")))
        (in-table (reverse-associations (assoc-table (input)))))
    (length (remove-duplicates (find-all-having key in-table ()) :test #'equal))))

(defun day-07-2020-2 ()
  (let ((for-key '((("shiny" "gold") 1)))
        (in-table (assoc-table (input))))
    (count-bags for-key in-table 0)))

(defun find-all-having (keys table found)
  (if (null keys) (return-from find-all-having found))
  (let ((new-keys
          (loop for i in keys with new-keys = ()
                do (setf new-keys (append (find-values i table) new-keys))
                finally (return new-keys))))
    (find-all-having new-keys table (append new-keys found))))

(defun find-values (key table)
  (loop for i in table when (equal (get-key i) key) collecting (get-number i)))

(defun get-key (value)
  (first value))

(defun get-number (bag)
  (second bag))

(defun count-bags (bags table count)
  (if (null bags) (return-from count-bags count))
  (destructuring-bind ((key multiplier) &rest rest) bags
    (let ((new-bags (rest (assoc key table :test #'equal))))
      (count-bags (append rest (update-count new-bags multiplier))
...
(defun to-list (input)
  (loop for line in (uiop:split-string input :separator (string #\linefeed))
        collecting (uiop:split-string line :separator " ")))

(defun assoc-table (input)
  (mapcar #'split-function (to-list input)))

(defun split-function (string)
  (normalize (split-at '("contain" "bags." "bags," "bag." "bag,") string () ())))

(defun normalize (list)
  (destructuring-bind (key &rest values) list
    (append (list (normalize-key key)) (normalize-values values))))

(defun normalize-key (key)
  (reverse (rest (reverse key))))

(defun normalize-values (values)
  (mapcar #'normalize-value (filter-out-no-other values)))

(defun filter-out-no-other (bags)
  (remove '("no" "other") bags :test #'equal))

(defun normalize-value (value)
  (destructuring-bind (number &rest key) value (list key (parse-integer number))))

(defun reverse-associations (assoc-table)
  (loop for i in assoc-table with reverted = () do
    (destructuring-bind (key &rest values) i
        (loop for v in values do
          (push (list (get-key v) key) reverted)))
        finally (return reverted)))

(defun split-at (at list front output)
  (if (null list) (return-from split-at (reverse output)))
  (if (member (first list) at :test #'string=)
      (progn
        (push (reverse front) output)
        (split-at at (rest list) () output))
      (split-at at (rest list) (push (first list) front) output)))
