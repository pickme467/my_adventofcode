(defun day-21-2020-1 ()
  (count-non-allergens (input)))

(defun day-21-2020-2 ()
  (let* ((hash (find-allergens (input)))
         (keys (sort (loop for k being the hash-key in hash collect k)
                     (lambda (x y) (string< (string x) (string y))))))
    (loop for i in keys for comma = "" then "," with output = ""
          do (setf output
                   (concatenate 'string output comma
                                (sb-unicode:lowercase (string (first (gethash i hash))))))
          finally (return output))))


(defun count-non-allergens (list)
  (let ((allergens (find-allergens-values list)))
    (loop for i in list
          sum (loop for ingr in i
                    count (and (not (listp ingr)) (not (member ingr allergens))) into output
                    finally (return output)))))

(defun find-allergens-values (input)
  (loop for v being the hash-values in (find-allergens input) append v))

(defun find-allergens (input)
  (let ((hash (reduce-map (make-allergen-map input))))
    (loop when (single-value hash)
            return hash
          do (loop for k being the hash-keys in hash using (hash-value v)
                   when (= 1 (length v))
                     do (remove-part (first v) hash)))))

(defun remove-part (value hash)
  (loop for k being the hash-key in hash using (hash-value v)
        when (> (length v) 1)
          do (setf (gethash k hash) (remove value v :test #'equal))))

(defun make-allergen-map (input)
  (let ((hash (make-hash-table :test #'equal)))
    (loop for i in input
          do (loop for key in (rest (first (last i)))
                   do (setf (gethash key hash)
                            (append (gethash key hash)
                                    (list (reverse (rest (reverse i))))))))
    hash))

(defun single-value (hash)
  (loop for v being the hash-value in hash
        when (> (length v) 1)
          return nil
        finally (return t)))

(defun reduce-map (hash)
  (let ((reduced (make-hash-table :test #'equal)))
    (loop for k being the hash-key in hash
          do (setf (gethash k reduced) (reduce (lambda (a b) (intersection a b :test #'equal))
                                               (gethash k hash))))
    reduced))
