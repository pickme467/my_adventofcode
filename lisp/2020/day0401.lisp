(defun day-04-2020-1 ()
  (count-2020 (input) #'is-good))

(defun day-04-2020-2 ()
  (count-2020 (input) #'is-even-better))

(defun count-2020 (input check)
  (loop for i in (uiop:split-string input :separator (string #\linefeed)) with passport = ""
        when (equal i "")
          count (funcall check passport) into valid
          and do (setf passport "")
        else
          do (setf passport (string-trim " " (concatenate 'string passport " " i)))
        finally (return valid)))

(defun is-good (passport)
  (let ((list (strip-values passport)))
    (or (equal list '("byr" "ecl" "eyr" "hcl" "hgt" "iyr" "pid"))
        (equal list '("byr" "cid" "ecl" "eyr" "hcl" "hgt" "iyr" "pid")))))

(defun is-even-better (passport)
  (unless (good passport) (return-from is-even-better nil))
  (loop for f in (separate-fields passport)
        unless (validate f)
          return nil
        finally (return t)))

(defun strip-values (passport)
  (sort (mapcar #'car (separate-fields passport)) #'string<))

(defun separate-fields (passport)
  (loop for i in (uiop:split-string passport :separator " ")
        collecting (uiop:split-string i :separator ":")))

(defun validate (field)
  (destructuring-bind (type value) field
    (cond ((equal type "byr") (validate-byr value))
          ((equal type "iyr") (validate-iyr value))
          ((equal type "eyr") (validate-eyr value))
          ((equal type "hgt") (validate-hgt value))
          ((equal type "hcl") (validate-hcl value))
          ((equal type "ecl") (validate-ecl value))
          ((equal type "pid") (validate-pid value))
          (t t))))

(defun validate-byr (date)
  (let ((d (parse-integer date))) (and (>= d 1920) (<= d 2002))))

(defun validate-iyr (date)
  (let ((d (parse-integer date))) (and (>= d 2010) (<= d 2020))))

(defun validate-eyr (date)
  (let ((d (parse-integer date))) (and (>= d 2020) (<= d 2030))))

(defun validate-hgt (hgt)
  (multiple-value-bind (height unit-index) (parse-integer hgt :junk-allowed t)
    (let ((unit (subseq hgt unit-index)))
      (or (and (equal unit "cm") (>= height 150) (<= height 193))
          (and (equal unit "in") (>= height 59) (<= height 76))))))

(defun validate-hcl (hcl)
  (let ((list (concatenate 'list hcl)))
    (multiple-value-bind (number length)
        (parse-integer (concatenate 'string (cdr list)) :radix 16 :junk-allowed t)
      (declare (ignore number))
      (and (equal #\# (car list)) (equal 6 (length (cdr list))) (equal 6 length)))))

(defun validate-ecl (ecl)
  (let ((valid '("amb" "blu" "brn" "gry" "grn" "hzl" "oth")))
    (member ecl valid :test #'equal)))

(defun validate-pid (pid)
  (multiple-value-bind (number length) (parse-integer pid :junk-allowed t)
    (declare (ignore number))
    (and (equal 9 (length pid)) (equal 9 length))))
