(defun day-02-2020-1 ()
  (loop for (min-max what where) in (input)
        count (valid-p min-max what where #'validator-1)))

(defun day-02-2020-2 ()
  (loop for (min-max what where) in (input)
        count (valid-p min-max what where #'validator-2)))

(defun validator-1 (min max what where)
  (let ((number (count (coerce what 'character) where)))
    (and (>= number min) (<= number max))))

(defun validator-2 (pos1 pos2 what where)
  (let ((found1 (equal what (get-at (1- pos1) where)))
        (found2 (equal what (get-at (1- pos2) where))))
    (and (or found1 found2)
         (not (and found1 found2)))))

(defun valid-p (min-max what where validator)
  (let* ((stng (symbol-name min-max))
         (min-next (multiple-value-list (parse-integer stng :junk-allowed t)))
         (min (car min-next))
         (max (parse-integer stng :start (1+ (cadr min-next))))
         (what-s (symbol-name what))
         (where-s (symbol-name where)))
    (funcall validator min max what-s where-s)))

(defun get-at (at where)
  (cond ((> at (length where)) "")
        (t (string (schar where at)))))
