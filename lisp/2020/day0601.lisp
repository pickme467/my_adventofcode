(defun day-06-2020-1 ()
  (traverse (input) #'sum-unique))

(defun day-06-2020-2 ()
  (traverse (input) #'check-intersection))

(defun traverse (input check)
  (loop for i in (uiop:split-string input :separator (string #\linefeed)) with answers = ()
        when (equal i "")
          sum (funcall check answers) into valid
          and do (setf answers ())
        else
          do (push i answers)
