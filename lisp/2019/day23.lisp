...
(defun build-network ()
  (let ((network (newhash)))
    (loop for i from 0 to 49 do
         (sethash i (calculate (list i) 0 (program-to-hash (input)) 0 ()) network))
    network))

(defun appendhash (key list hash)
  (sethash key (append (gethash key hash) list) hash))

(defun getmessage (key hash nat)
  (cond
    ((and (= key 0) (= 0 (hash-table-count hash)))
     (let ((nat-value (gethash 255 nat)))
       (cond ((not (null (gethash (cadr nat-value) nat)))
              (list 'found-it (cadr nat-value) nil))
             (t
              (sethash (cadr nat-value) nat-value nat)
              nat-value))))
    (t
     (let ((message (gethash key hash)))
       (cond ((null message) '(-1))
             (t message))))))

(defun send-second-time-same-y-nat-p (hash nat)
  (cond
...
(defun route (network &optional (nat (newhash)))
  (let ((messages (newhash)))
    (loop for (nil output) being the hash-values in network do
         (loop for (a x y) in (make-three (reverse output))
            do (cond
                 ((= a 255) (sethash a (list x y) nat))
                 (t (appendhash a (list x y) messages)))))
    (cond
      ((send-second-time-same-y-nat-p messages nat)
       (getmessage 0 messages nat))
      (t
       (loop for i from 0 to 49 do
            (destructuring-bind (operation o1 start program relative o2) (gethash i network)
              (declare (ignore operation o1 o2))
              (sethash i (calculate
                          (getmessage i messages nat) start program relative ()) network)))
       (if (not (null (gethash 255 nat)))
           (progn
             (append (gethash 255 nat) (list network)))
           network)))))

(defun do-route (network)
  (let ((n network))
    (loop do
         (setf n (route n))
         (if (listp n) (return n)))))

(defun do-route-with-nat (network)
  (let ((n network)
        (nat (newhash)))
    (loop do
         (if (listp n)
             (progn
               (destructuring-bind (x y network) n
                 (cond ((equal x 'found-it)
                        (return y))
                       (t
                        (sethash 255 (list x y) nat)
                        (setf n (route network nat))
                        ))))
             (setf n (route n nat))))))

(defun make-three (list)
  (loop for a in list by #'cdddr
     for b in (cdr list) by #'cdddr
       for c in (cddr list) by #'cdddr collecting (list a b c)))

;; day 23, part 1
(time (assert (= 26744 (cadr (do-route (build-network))))))

;; day 23, part 2
(time (assert (= 19498 (do-route-with-nat (build-network)))))
