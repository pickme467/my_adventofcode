...
(defun distance (command)
  (parse-integer  (subseq command 1)))

(defun coordinates (start command)
  (let ((x (nth 0 start)) (y (nth 1 start)) (dir (direction command)) (len (distance command)))
    (cond
      ((equal 'R dir) (list (vector x y (+ x len) y) (list (+ x len) y)))
      ((equal 'L dir) (list (vector (- x len) y x y) (list (- x len) y)))
      ((equal 'U dir) (list (vector x (- y len) x y) (list x (- y len))))
      ((equal 'D dir) (list (vector x y x (+ y len)) (list x (+ y len)))))))

(defun draw-map (elements)
  (let ((coords (list 0 0)))
    (list (map 'list #'(lambda (element)
                         (let* ((output (coordinates coords element)))
                           (format t "~A~%" output)
                           (setf (nth 0 coords) (nth 0 (nth 1 output)))
                           (setf (nth 1 coords) (nth 1 (nth 1 output)))
                           (list (nth 0 output)  coords))) elements))))
