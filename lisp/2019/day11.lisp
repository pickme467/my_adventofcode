...
(defun calculate (input start program output relative)
    ...
    (multiple-value-bind (opcode p1 p2 p3) (decode-command command)
      (cond
        ...
        ((= opcode 1)
         ...
         (calculate input (+ 4 start) program output relative))
        ((= opcode 2)
         (do-multiply p1 p2 p3 relative start program)
         (calculate input (+ 4 start) program output relative))
        ((= opcode 3)
         (if (null input)
             (vector start program output relative)
             (progn
               (do-input (pop input) p1 relative start program)
               (calculate input (+ 2 start) program output relative))))
        ((= opcode 4)
         (let ((new-output (push (do-output p1 relative start program) output)))
           (format t "Output: ~a~%" new-output)
           (calculate input (+ 2 start) program new-output relative)))
        ((= opcode 5)
         (calculate input (do-jump-if-true p1 p2 relative start program) program output relative))
        ((= opcode 6)
         (calculate input (do-jump-if-false p1 p2 relative start program) program output relative))
        ((= opcode 7)
        ...
        ((= opcode 8)
         ...
         (calculate input (+ 4 start) program output relative))
        ((= opcode 9)
         (calculate input (+ 2 start) program output (do-relative p1 relative start program)))
        (t output)))))
...
;; ;; day 9 part 1
;; (time (assert (= 2204990589 (calculate 0 (list-to-hash (input)) '(1) 0 0))))

;; ;; day 9 part 2
;; (time (assert (= 50008 (calculate 0 (list-to-hash (input)) '(2) 0 0))))
