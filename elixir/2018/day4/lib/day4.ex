defmodule Day4 do
  ...
  """
  def solution_1 do
    guard = find_guard_that_sleeps_the_most()

    minute = find_minute_for(guard)

    find_guard_number(guard) * minute
  end
  ...
  """
  def solution_2 do
    guard = find_guard_with_same_minute_slept_the_most()
    minute =
      guard
      |> elem(1)
      |> elem(0)
  ...
  defp find_guard_number(guard) do
      guard
      |> elem(0)
      |> String.slice(1..-1)
      |> String.to_integer()
  ...
  defp find_guard_that_sleeps_the_most() do
    get_all_guards_data()
    |> Enum.sort_by(
      fn
        {_k, {sleep_time_total, _}} -> sleep_time_total
      end,
      &>=/2
    )
    |> hd()
  ...
  defp get_all_guards_data() do
    Day4.Input.get_input()
    |> Enum.map(&parse/1)
    |> Enum.reduce({%{}, {:no_guard, :no_time}}, &get_guards_data/2)
    |> elem(0)
  end

  defp find_guard_with_same_minute_slept_the_most() do
    get_all_guards_data()
    |> Enum.map(fn
      ({guard, {_, minutes}}) ->
        minute_slept_the_most =
          minutes
          |> Enum.reduce(%{}, &count_minutes_slept/2)
          |> Enum.sort_by(fn ({_minute, count}) -> count end, &>=/2)
          |> hd()
      {guard, minute_slept_the_most}
    end)
    |> Enum.sort_by(fn ({_guard, {_minute, count}}) -> count end, &>=/2)
    |> hd()
  end

  defp find_minute_for(guard) do
    guard
    |> elem(1)
    |> elem(1)
    |> Enum.reduce(%{}, &count_minutes_slept/2)
    |> Enum.sort_by(
      fn
        ({_minute_start, sleep_time_total}) -> sleep_time_total
      end,
      &>=/2
    )
    |> hd()
  ...
  defp count_minutes_slept({sleep_start, sleep_end}, map) do
    sleep_start..(sleep_end - 1)
    |> Enum.reduce(map, fn x, map ->
      Map.put(map, x, Map.get(map, x, 0) + 1)
    end)
  end

  defp parse(line) do
    [date, command] = String.split(line, "]")
    {get_date(date), get_command(command)}
  end

  defp get_date(date) do
    "[" <> date = date
    [day, hour] = String.split(date)
    [_hour, minute] = String.split(hour, ":")
    {day, String.to_integer(minute)}
  end

  defp get_command(" falls asleep"), do: :sleep
  defp get_command(" wakes up"), do: :wake_up

  defp get_command(guard) do
    [_guard, id | _rest] = String.split(guard, " ", trim: true)
    id
  end

  defp get_guards_data({date, guard}, {map, _last_guard})
       when is_binary(guard) do
    {map, {guard, date}}
  end

  defp get_guards_data({date, :sleep}, {map, {guard, _date}}) do
    {map, {guard, date}}
  end

  defp get_guards_data(
         {{day, hour_w}, :wake_up},
         {map, {guard, {day, hour_s}}}
       ) do
    {slept_already, times} = Map.get(map, guard, {0, []})
    slept_this_time = hour_w - hour_s

    {
      Map.put(
        map,
        guard,
        {slept_already + slept_this_time, [{hour_s, hour_w}] ++ times}
      ),
      {guard, {day, hour_s}}
    }
  end
