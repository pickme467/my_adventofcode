defmodule Day10 do
  ...
  @doc """
  This part is a bit fake, since I don't have implementation of letter
  recognition, so the solution is based on the minimum (or exact)
  height of the row being 9 characters. Algorithm looks up for
  sequence where max_y - min_y == 9.
  """
  def part_1_and_2 do
    expected_width = 9
    ...
    try do
      1..20_000
      |> Enum.reduce(Day10Input.input(), fn n, lines ->
        {new_lines, width} = do_steps(lines)

        case width == expected_width do
          true -> throw({new_lines, width, n})
          false -> new_lines
        end
      end)
    catch
  ...
  def print(lines) do
    {{min_x, min_y}, {max_x, max_y}} = minmax(lines)

    empty_line =
      min_x..max_x
      |> Enum.reduce(" ", fn _i, acc -> " " <> acc end)

    matrix =
      (min_y - 1)..max_y
      |> Enum.reduce(%{}, fn index, map ->
        Map.put(map, index, empty_line)
      end)

    matrix =
      lines
      |> Enum.map(fn {x, _v} -> x end)
      |> Enum.reduce(matrix, fn {x, y}, map ->
        line = Map.get(map, y)
        {s, e} = String.split_at(line, x - min_x)
        Map.put(map, y, s <> String.replace_prefix(e, " ", "#"))
      end)

    Enum.each((min_y - 1)..max_y, fn i ->
      IO.puts(Map.get(matrix, i))
    end)
  end

  defp do_steps(lines) do
    new_lines = one_step(lines)
    width = width(new_lines)
    {new_lines, width}
  end

  defp one_step(lines) do
    lines
  ...
  defp width(lines) do
    {{_, min_y}, {_, max_y}} = minmax(lines)
    max_y - min_y
  end

  defp minmax(lines) do
    lines =
      lines
      |> Enum.map(fn {x, _v} -> x end)

    lines
    |> Enum.reduce({hd(lines), hd(lines)}, fn {x, y}, {{min_x, min_y}, {max_x, max_y}} ->
...
defmodule Day10Input do
  def input do
    ...
    |> Enum.map(fn line ->
      [pos, velocity] =
        line
        |> String.trim_leading("position=<")
        |> String.trim_trailing(">")
        |> String.split("> velocity=<")

      {make_vector(pos), make_vector(velocity)}
    end)
  end

  defp make_vector(pos) do
    [x, y] = String.split(pos, ", ")
    {make_integer(x), make_integer(y)}
  end

  defp make_integer(x) do
    x
    |> String.trim()
    |> String.to_integer()
  end
